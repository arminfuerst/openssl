...
# run this test with:
# export TESTS='test_ca-updatedb'; make tests
# from the base SSL dir
...
ok(
    run(cmd(['true'])), 'true-test'
  );

ok(
    run(cmd(["faketime '1999-12-30 17:31:12' date +%Y%m%d-%H%M%S > afresult.txt"])), 'faketime'
  );
#        ok(
#            run(app(['openssl',
#                     'req',
